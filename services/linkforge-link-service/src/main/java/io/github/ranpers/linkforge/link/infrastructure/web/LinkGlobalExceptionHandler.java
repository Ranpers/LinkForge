 ...
 *
 * @implNote 最低优先级确保功能级异常映射先于全局兜底执行。这里刻意不为
...
public class LinkGlobalExceptionHandler {
    ...
    ProblemDetail invalidQueryParameter(InvalidQueryParameterException exception) {
        return invalidRequest(exception.getMessage());
    }
    ...
    ProblemDetail methodNotSupported(HttpRequestMethodNotSupportedException exception) {
        return ApiProblems.create(
