...
@Component
public class KafkaLinkControlEventPublisher implements LinkControlEventPublisher {

    private final KafkaTemplate<String, String> kafkaTemplate;
    private final String topic;

    public KafkaLinkControlEventPublisher(
    ...
    public void publish(PendingOutboxEvent event, Duration timeout) {
        try {
            ...
            throw new LinkControlEventPublishException("Kafka 投递等待被中断: eventId=" + event.id(), exception);
        } catch (ExecutionException | TimeoutException exception) {
            throw new LinkControlEventPublishException("Kafka 投递失败: eventId=" + event.id(), exception);
        }
    }
