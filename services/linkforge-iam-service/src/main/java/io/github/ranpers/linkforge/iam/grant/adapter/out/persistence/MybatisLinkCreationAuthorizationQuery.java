...
public class MybatisLinkCreationAuthorizationQuery implements LinkCreationAuthorizationQuery {
    ...
    @Override
    public LinkCreationAuthorizationSnapshot load(UUID userId, UUID shortDomainId) {
        LinkCreationAuthorizationRow row = mapper.findSnapshot(userId, shortDomainId);
        if (row == null) {
