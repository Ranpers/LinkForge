 ...
 * 归一化后的值同时满足 {@code t_domain} 的 {@code domain} 列语义与 link-service
 * {@code t_domain_state.host} 的 {@code CHECK (host = lower(host) AND host !~ '[/:]')} 约束，
 * 因此以本类型构造的值可以安全写入跨服务控制事件。</p>
 *
 * @param value 归一化后的主机名，保证为小写、至少两个标签、总长不超过 253
 */
public record DomainHost(String value) {

    /** 与 {@code t_domain.domain} 以及 {@code t_domain_state.host} 的列宽一致。 */
    ...
    public DomainHost {
        if (value == null || value.isBlank()) {
            throw new InvalidDomainHostException("域名不能为空");
        }
        String normalized = value.trim().toLowerCase(Locale.ROOT);
        if (normalized.length() > MAX_LENGTH) {
            throw new InvalidDomainHostException("域名长度不能超过 " + MAX_LENGTH);
        }
        if (!FORMAT.matcher(normalized).matches()) {
            throw new InvalidDomainHostException(
