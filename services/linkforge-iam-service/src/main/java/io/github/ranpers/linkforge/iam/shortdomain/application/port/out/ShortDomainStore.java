 ...
 */
public interface ShortDomainStore {

    /**
     * 插入一个启用状态的域名、把该域名直接授权给创建者，并追加一条
     * {@code ShortDomainAvailabilityChanged} 控制事件，使 link-service 建立解析用的状态投影。
     *
     * <p>三件事在同一条语句内完成，避免出现「域名已建但事件未发」的中间态：
     * 缺少事件会让该域名下的短链创建因 link-service 的外键约束失败。</p>
     *
     * @param actorUserId 操作者；需具备 {@code short-domain:create} 且账号状态正常
     * @param host        已归一化的主机名
     * @param name        可为空的展示名称
     ...
     * 更新域名的展示名称。
     *
     * @param actorUserId 操作者；需具备 {@code short-domain:update} 且账号状态正常
     * @param shortDomainId    目标域名标识
     * @param name        新的展示名称；为 {@code null} 表示清空
     * @return 更新、无变化、目标不存在与权限拒绝四态结果
     */
    UpdateResult update(UUID actorUserId, UUID shortDomainId, String name);
     ...
     */
    record CreationOutcome(CreationResult result, ShortDomainListItem created) {
    }
}
