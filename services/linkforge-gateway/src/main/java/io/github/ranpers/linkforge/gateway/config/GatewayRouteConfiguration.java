...
public class GatewayRouteConfiguration {
     ...
     * 前者是公开、只读、可缓存的 OIDC 与 JWK 元数据，端点本身不接收用户凭据，也不执行登录或令牌
     * 签发，因此没有口令爆破面；放行不会绕过认证授权，也不会暴露受保护数据，代价只是这段时间里
     * 失去流量洪泛与资源消耗的限流保护，而拒绝降级会让所有依赖它的资源服务器与 OIDC 客户端一起
     * 失效，比较之下这个代价更大。跳转路由确实有流量洪泛、短码枚举与下游容量风险，这里是自觉地
     * 以可用性优先。
     * <p>
