...
public class GatewayRouteConfiguration {
     ...
     * 比无保护放行更稳妥。跳转路由取 {@code ALLOW}，在限流基础设施故障时优先维持已有短链
     * 可用性；它同样有流量洪泛、短码枚举与下游容量风险，这里是自觉地以可用性优先。
     * <p>
     * 策略按路由声明，不能按路径前缀推断：将来若 {@code link-api} 下出现纯公开、只读、以可用性
     * 优先的接口，应拆成独立路由并单独指定策略，而不是沿用该前缀现有的 {@code REJECT}。
